    ...
    render() {
        return (
            <div className="min-h-screen bg-gray-100 pt-10 px-6 sm:pl-32 sm:pt-20">
                <h1 className="text-2xl font-medium select-none"><span role="img"
                                                                       aria-label="books">📚</span> Memory<span
                    className="font-bold">CLIP</span></h1>
                    ...
                    {this.state.results.map((val, index) => (
                        <div className="my-2 rounded">
                            <Link to={"/article/" + val.id}>
                                <div key={index}
                                     className="select-none flex pl-1 p-2 bg-gray-100 border-gray-400 cursor-pointer rounded border-1px shadow hover:shadow-md trans">
